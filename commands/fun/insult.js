...
class insultCommand extends commando.Command {
  ...
  async run(message, args) {
    message.delete();
    let insultMember = message.guild.member(message.mentions.users.first());
    if (!insultMember) {
      return message.reply(` you ${start[Math.floor(Math.random() * start.length)]} ${middle[Math.floor(Math.random() * middle.length)]} ${end[Math.floor(Math.random() * end.length)]}.`)
        .then(response => response.delete(5000).catch(error => console.log(error.stack))).catch(error => console.log(error.stack));
    } else {
      message.channel.send(`${insultMember},  you ${start[Math.floor(Math.random() * start.length)]} ${middle[Math.floor(Math.random() * middle.length)]} ${end[Math.floor(Math.random() * end.length)]}.`)
    }
  }
}
